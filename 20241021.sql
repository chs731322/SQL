-- INDEX
-- 테이블에 있는 데이터를 빠르게 검색하기 위한 용도로 나온 데이터베이스 객체
-- 테이블에 하나 이상의 컬럼으로 인덱스를 만들 수 있음
-- 오라클에서는 기본적으로 테이블 기본키로 인덱스가 설정되어 있음

-- 생성한 모든 인덱스 객체 조회
SELECT * FROM USER_INDEXES;

-- CREATE [UNIQUE] INDEX 인덱스명 ON 테이블명(컬럼1, 컬럼2, ...);
-- UNIQUE 키워드를 사용하면 해당 컬럼에 중복된 값이 없도록 보장

-- PERSON 테이블에 사람 이름으로 인덱스 지정
CREATE INDEX PERSON_PNAME_IDX ON PERSON(PNAME);
-- 인덱스 삭제
DROP INDEX PERSON_PNAME_IDX;

-- 리빌딩 작업
-- 데이터를 추가, 삭제, 수정 등의 작업을 하다보면 트리가 한쪽으로 치우쳐지는 현상이 나기 때문
ALTER INDEX PERSON_PNAME_IDX REBUILD;

-- CAR_SELL 테이블에 판매 날짜에 인덱스 적용
CREATE INDEX CAR_SELL_DATE_IDX ON CAR_SELL(CAR_SELL_DATE);

-- 사원 테이블에서 이름, 직급, 부서 인덱스를 적용
CREATE INDEX EMP_IDX ON EMPLOYEE(EMP_NAME, POS_NO, DEPT_NO);

-- SEQUENCE(시퀸스)
-- 자동으로 증가하는 순번을 반환하는 데이터베이스 객체
CREATE SEQUENCE NO_SEQ;
DROP SEQUENCE TEST_SEQ;

SELECT NO_SEQ.NEXTVAL FROM DUAL;
SELECT NO_SEQ.CURRVAL FROM DUAL;

CREATE SEQUENCE TEST_SEQ
INCREMENT BY 2
START WITH 4
MINVALUE 4 --NOMINVALUE (1)
MAXVALUE 10 --NOMAXVALUE
CYCLE --NOCYCLE
CACHE 2; --NOCACHE

SELECT TEST_SEQ.NEXTVAL FROM DUAL;
SELECT TEST_SEQ.CURRVAL FROM DUAL;

-- 생성한 시퀸스 목록 조회
SELECT * FROM USER_SEQUENCES;

/*
문제 1: 기본 시퀀스 생성
시퀀스 이름 : SEQ_EMP_ID
시작값 1
최대 9999
증가 1
캐시 20
*/

CREATE SEQUENCE SEQ_EMP_ID
INCREMENT BY 1
START WITH 1
MAXVALUE 9999
CACHE 20;

SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL;

/*
문제 2
시퀀스 이름 : SEQ_DEPT_ID
시작값 100
최대 110
최소 100
증가 1
CYCLE
*/

CREATE SEQUENCE SEQ_DEPT_ID
START WITH 100
MINVALUE 100
MAXVALUE 110
INCREMENT BY 1
CYCLE
CACHE 2;

SELECT SEQ_DEPT_ID.NEXTVAL FROM DUAL;

/*
시퀀스 이름 : SEQ_ORDER_NO
시작 값 1000
최소 500
감소 10
NOCYCLE
*/
CREATE SEQUENCE SEQ_ORDER_ID
START WITH 1000
MINVALUE 500
MAXVALUE 2000
INCREMENT BY -10
CYCLE 
CACHE 2;

SELECT SEQ_ORDER_ID.NEXTVAL FROM DUAL;

-- VIEW(뷰)
-- SQL에 하나 이상의 테이블의 조회 결과를 저장한 가상 테이블, 스토리지 용량 X
-- 실제 데이터를 저장하지 않고, 쿼리의 결과를 미리 정의해 두어 필요할 때 재사용
CREATE OR REPLACE VIEW 뷰이름
AS
조회할 SQL문(SELECT문)

-- 뷰 생성 권한 (SYSTEM 접속)
GRANT CREATE VIEW TO C##SCOTT;

-- 관리자 권한
GRANT DBA TO 사용자명;

-- 학생 정보 조회, 학번, 이름, 학과명, 평점, 성별 조회하는 조회문 작성
CREATE OR REPLACE VIEW STUDENT_VIEW
AS
SELECT 
	S.STD_NO, S.STD_NAME, M.MAJOR_NAME, S.STD_SCORE, S.STD_GENDER
FROM STUDENT S JOIN MAJOR M ON S.MAJOR_NO = M.MAJOR_NO;

-- 뷰로 조회
SELECT * FROM STUDENT_VIEW;

-- 성별 컬럼 제거 후 조회 (DROP 할 필요 없이 다시 만들면 됨)
CREATE OR REPLACE VIEW STUDENT_VIEW
AS
SELECT 
	S.STD_NO, S.STD_NAME, M.MAJOR_NAME, S.STD_SCORE
FROM STUDENT S JOIN MAJOR M ON S.MAJOR_NO = M.MAJOR_NO;

-- 학과별 인원수
SELECT MAJOR_NAME,COUNT(*)
FROM STUDENT_VIEW
GROUP BY MAJOR_NAME;

-- 장학금을 받는 학생들의 정보를 저장하는 뷰 생성
-- 장학금 번호, 금액, 학번, 학생 이름, 학과명, 성별, 평점
CREATE OR REPLACE
VIEW STUDENT_SCH_VIEW
AS
SELECT
	SS.SCHOLARSHIP_NO,
	SS.MONEY,
	S.STD_NO,
	S.STD_NAME, 
		M.MAJOR_NAME,
	S.STD_GENDER,
	S.STD_SCORE
FROM
	STUDENT S
JOIN STUDENT_SCHOLARSHIP SS ON
	S.STD_NO = SS.STD_NO
JOIN MAJOR M ON
	S.MAJOR_NO = M.MAJOR_NO
ORDER BY SS.SCHOLARSHIP_NO ASC;

SELECT * FROM STUDENT_SCH_VIEW;
